 ...
 *              It provides an Ext Container for a summary page for 'exec' type
 *              queries made for an application.
 *
 * Licence:
...
return function (appName, extra) {
    ...
    var queryProfilesStore = new Ext.data.JsonStore ({
        proxy: new Ext.data.HttpProxy ({ url: jarvisUrl ('queries_profile/' + appName + '/' + extra.query), method: 'GET' }),
        autoLoad: true,
        root: 'data',
    ...
    return new Ext.Panel ({
        title: appName + ' - Datasets (exec) - ' + extra.query,
        layout: 'border',
        hideMode: 'offsets',
        closable: true,
        items: [
