...
sub fetchall_arrayref {
    ...
    my @column_names = map { my $tmp = $_->{'Member'}->{'Caption'}->content } $root->{'Axes'}->{'Axis'}[0]->{'Tuples'}{'Tuple'}('@');
    my $num_columns = scalar @column_names;

    my @row_names = map { my $tmp = $_->{'Member'}->{'Caption'}->content } $root->{'Axes'}->{'Axis'}[1]->{'Tuples'}{'Tuple'}('@');
